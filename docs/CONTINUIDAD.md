...
Última compilación posterior a la revisión nativa inicial: regla de 72 horas y corrección de cancelación tardía. `test lint assembleDebug :app:assembleValidation assembleRelease --offline` correcto, 82 pruebas por variante y lint app sin errores/16 advertencias. APK oficial firmada preparada en `dist/1.12.0/`; debug final en `dist/Michi-Musica-1.12.0-debug-arm64.apk`. Hashes en la entrada de cierre del historial. **Estos últimos cambios aún no están instalados ni publicados:** Pixel conectado pero bloqueado. La batería aislada ya comprobó seguimiento, cancelación activa/en cola, reintento y cancelación tardía/durante commit. Android rechazó el siguiente inicio de descarga al seguir bloqueado el móvil; falta repetir la batería completa con pantalla desbloqueada. Continuar con `scripts/validate_podcasts.ps1`, que usa exclusivamente la app aislada `.validation`, y después actualizar la app personal mediante `adb install -r`. Leer [la batería nativa](VALIDACION_PODCASTS.md). No confundir arnés compilado con pruebas nativas ejecutadas.
