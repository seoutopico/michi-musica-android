...
- RSS público HTTPS, programas persistentes, novedades por identidad y marcas vistas independientes de la escucha. Actualizar manualmente, comprobación periódica cada seis horas aproximadas y avisos opcionales. Añadir valida antes de seguir; quitar conserva audios/progreso. No hay cuentas, pagos ni entrada de credenciales.
- Descargas manuales mediante servicio en primer plano: cola, progreso, cancelación, reintento, archivo temporal privado y copia SAF con nombre disponible. No se hace visible un documento `.part` en Biblioteca. Validación de pista de audio y duración; las posiciones siguen usando el servicio de reproducción existente. Catálogo privado y snapshot atómico; sin edición de etiquetas.
- Filtros de mecenas/fans/pago, adelantos/tráilers identificados y metadatos de acceso de pago. Límites de XML/red, DTD/entidades externas deshabilitados y rechazo de URLs locales/privadas o con credenciales. Portadas en caché muestreada; nuevas conexiones y permisos documentados en PRIVACY.md.
- Datos reales: el parser Kotlin admite 209 de las 250 entradas del RSS de Días Extraños y excluye 41 exclusivas. El cliente HTTPS Kotlin obtiene HTTP 200/audio-mpeg para la muestra pública. **No equivale a 209 audios completos verificados:** la muestra «Sectas 3.0» anuncia 540 s pero entrega 126,354 s según ffprobe; la comprobación de duración rechaza ese recorte. RSS de Sergio Parra: HTTP 403, audio no validado. Límites completos en PODCASTS_RSS_1_12.md.
- `gradlew test lint assembleDebug --offline`, JDK 21: **BUILD SUCCESSFUL en 1m 12s**. **80 pruebas debug y 80 release**, sin fallos ni errores (25 nuevas). Lint app: **0 errores/16 advertencias**, las mismas advertencias preexistentes; iconpack conserva 0 errores/3 advertencias. No se actualizan SDK ni dependencias. Previews de producción compiladas para Siguiendo/Novedades/vacío y Rosa con texto grande.
- APK debug 1.12.0 (22), firma debug verificada con apksigner: `dist/Michi-Musica-1.12.0-debug-arm64.apk`. SHA-256: `76017e59c7291f8521c3f444de8c8c41ef2f68ffcdf3d10c2a7a507b920a08ca`. Los artefactos de dist no se versionan.
- **Instalada en Pixel 7 mediante actualización, sin borrar datos.** Corrección nativa del campo RSS: desactivar autocorrección para evitar espacios en el dominio. Verificados seguimiento/búsqueda de Días Extraños, rechazo del recorte, descarga completa NASA por SAF, clasificación y metadatos, reproducción en segundo plano y recuperación de posición en pausa tras reiniciar. Revisadas capturas reales en Medianoche y Rosa con texto al 130 %. JobScheduler acepta la ejecución forzada; avisos opcionales siguen desactivados.
- Retirados programa y audio NASA de validación; Días Extraños queda seguido. Catálogo personal idéntico y última escucha restaurada. Los 112 archivos originales inventariados de la raíz conservan ruta, tamaño y fecha; sin audio de prueba ni temporales. Restaurados tema y ajustes de pantalla. Ficha PODCASTS_RSS_1_12.md detalla evidencias y límites: cancelación/reintento/interrupción, permisos/espacio, avisos futuros, Doze y pantalla bloqueada siguen pendientes en MMA-039. No publicada como release estable.
