...
- `gradlew test lint assembleDebug --offline`, JDK 21: **BUILD SUCCESSFUL en 1m 10s**. **80 pruebas debug y 80 release**, sin fallos ni errores (25 nuevas). Lint app: **0 errores/16 advertencias**, las mismas advertencias preexistentes; iconpack conserva 0 errores/3 advertencias. No se actualizan SDK ni dependencias. Previews de producción compiladas para Siguiendo/Novedades/vacío y Rosa con texto grande.
- APK debug 1.12.0 (22), firma debug verificada con apksigner: `dist/Michi-Musica-1.12.0-debug-arm64.apk`. SHA-256: `83d7c1d3ba51a112c0a39873ade931e2c60a3e1dcc5cadd741c7338eaa652490`. Los artefactos de dist no se versionan.
- **No instalada ni publicada.** ADB no detecta ningún dispositivo; no hay emulador configurado. Se inspeccionó la captura anterior de Podcasts y se contrastó el código con su ficha, pero no se ha renderizado/revisado la nueva pantalla en Android. Pendiente MMA-039: validación visual/táctil, reproducción, descarga completa, SAF, cancelación/reintento/reinicio y avisos en Pixel. No se han tocado audios, datos ni preferencias del móvil.
